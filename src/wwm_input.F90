...
!**********************************************************************
      ...
      USE DATAPOOL
      IMPLICIT NONE
      NAMELIST /ICE/ LICEFROMWWM, ICE_NETCDF_FILE,& 
     & LSAVE_INTERP_ARRAY_ICE, &
     & ICENCVAR, ICEXNCVAR, ICEYNCVAR, EXTRAPOLATION_ALLOWED_ICE
      INTEGER ios

     !the 2 calls below are fail safe
      READ(INP%FHNDL, NML = ICE, iostat=ios)
      REWIND(INP%FHNDL)
