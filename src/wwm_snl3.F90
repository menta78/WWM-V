...
!**********************************************************************
      ...
      !write(*,*) 'constants', c1, c2, c3
      !write(*,*) 'parameters', TRI_ARR
      !write(*,*) 'biphase', BIPH, SINBPH
      !write(*,*) 'dep', DEP(IP)
      !pause

      !write(*,*) TRI_ISBEGIN, ISMAX
      ...
      DO ID = 1, NUMDIR
        DO IS = 1, NUMSIG
          SIGPI = SPSIG(IS) * PI2
          IF (WALOC(IS,ID) .LT. THR) CYCLE
          STRI = SA(IS,ID) - TWO*(TRI_WISP*SA(IS+TRI_ISP1,ID) + TRI_WISP1*SA(IS+TRI_ISP,ID))
          IF (ABS(STRI) .LT. THR) CYCLE
          eCont = STRI / SIGPI
          IF (STRI .GT. 0.) THEN
            SSNL3(IS,ID)  = eCont
            DSSNL3(IS,ID) = zero!eCont/WALOC(IS,ID)
          ELSE
            SSNL3(IS,ID)  = eCont
            DSSNL3(IS,ID) = eCont/WALOC(IS,ID)
          END IF
        END DO
      END DO
